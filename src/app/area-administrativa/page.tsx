...
export default function AreaAdministrativa() {
  ...
  const autorizarAdmin = async (nome: string, email: string) => {
    ...
    const { error } = await supabase
      .from("administradores")
      .upsert({ email: emailLower, nome: nomeNormalizado }, { onConflict: "email" });
...
- Status do Parecer: Recomendado para arremate sem restrições.`;
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f6] text-[#111111] flex flex-col font-sans">
      ...
      {/* Barra de Navegação Mobile — visível apenas em telas pequenas */}
      <div className="lg:hidden sticky top-[65px] z-20 bg-white border-b border-gray-200 shadow-sm">
        <nav className="flex overflow-x-auto scrollbar-hide gap-1.5 px-4 py-2.5">
          {[
            { key: "visao-geral", label: "Visão Geral", icon: <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg> },
            { key: "solicitacoes", label: "Solicitações", icon: <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg> },
            { key: "destaque", label: "Destaque", icon: <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.907c.961 0 1.36 1.243.577 1.835l-3.97 2.895a1 1 0 00-.364 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.971-2.895a1 1 0 00-1.175 0l-3.97 2.895c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.364-1.118l-3.97-2.895c-.783-.592-.383-1.835.57-1.835h4.907a1 1 0 00.95-.69l1.519-4.674z" /></svg> },
            { key: "oportunidades", label: "Oportunidades", icon: <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg> },
            { key: "usuarios", label: "Usuários", icon: <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg> },
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setTabAtiva(tab.key as any)}
              className={`flex-shrink-0 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-extrabold transition-all ${
                tabAtiva === tab.key
            ...
            <div className="grid gap-6 lg:grid-cols-4 items-start">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white shadow-xs overflow-hidden">
                ...
                >
                  ...
                  <svg
                    className={`h-4 w-4 text-gray-400 transition-transform lg:hidden ${mobileFiltersOpen ? "rotate-180" : ""}`}
                ...
                <div className={`px-5 pb-5 space-y-3 ${mobileFiltersOpen ? "block" : "hidden lg:block"}`}>
                  <div>
